...
def get_user_input() -> dict:
    """사용자로부터 사주 분석에 필요한 정보를 입력받습니다."""
    print("\n--- 사주 분석 정보를 입력해주세요 ---")
    year = input("- 생년 (예: 1995): ")
    month = input("- 생월 (예: 10): ")
    day = input("- 생일 (예: 20): ")
    hour = input("- 생시 (0-23 사이, 예: 11): ")
    location = input("- 태어난 지역 (예: 서울): ")
    question = input("- 궁금한 점이 있으면 입력해주세요 (예: 오늘의 행운의 색상은?): ")
    return {
        "year": year,
        "month": month,
        "day": day,
...
if __name__ == "__main__":
    try:
        ...
        # 2. Gemini 프롬프트 생성
        prompt = generate_saju_prompt(user_info)
        
        # 3. Gemini API 호출 및 결과 받기
        saju_data = get_saju_analysis_from_gemini(prompt)
        final_data = json.loads(extract_json_from_string(saju_data))
        print(final_data)
        #display(Markdown(f"Response:\n {saju_data.text}"))
        
        #print(type(saju_data))
